...
public class CollectionServiceImpl implements CollectionService {
     ...
     */
    @Override
    public FCResponse<Boolean> isCollect(Long productId) {
        LoginInfoData loginInfoData = AuthInterceptor.getLoginInfo();
        if (ObjectUtils.isEmpty(loginInfoData)) {
            throw new FCException("登陆失效");
        }
        Long memberId = loginInfoData.getId();
        Integer count = this.collectionMapper.isCollect(memberId, productId);
        return FCResponse.dataResponse(HttpFcStatus.DATASUCCESSGET.getCode(), "查询成功", count > 0);
    }
     ...
     */
    @Override
    public FCResponse<Void> cancelCollect(Long productId) {
        LoginInfoData loginInfoData = AuthInterceptor.getLoginInfo();
        if (ObjectUtils.isEmpty(loginInfoData)) {
            throw new FCException("登陆失效");
        }
        Long memberId = loginInfoData.getId();
        this.collectionMapper.cancelCollect(memberId, productId);
        return FCResponse.dataResponse(HttpFcStatus.DATASUCCESSGET.getCode(), HttpMsg.collection.COLLECTION_CANCEL_SUCCESS.getMsg());
    }
     ...
     */
    @Override
    public FCResponse<Long> getCollectCountByProductId(Long productId) {
        Long count = this.collectionMapper.getCollectCountByProductId(productId);
        return FCResponse.dataResponse(HttpFcStatus.DATASUCCESSGET.getCode(), HttpMsg.collection.GET_SUCCESS.getMsg(), count);
    }
}
