...
public class ProductListener {
    ...
    /**
     * 商品审核通过后导入到ElasticSearch中
     *
     * @param elSearchItemJson
     */
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = "CODEWORLD.CLOUD.EL.PRODUCT.IMPORT.QUEUE", declare = "true"),
            exchange = @Exchange(value = "CODEWORLD-SHOP.EXCHANGE", ignoreDeclarationExceptions = "true", type = ExchangeTypes.TOPIC),
            key = {"el.product.import"}
    ))
    public void importGoods(String elSearchItemJson) {
        if (ObjectUtils.isEmpty(elSearchItemJson)) {
            log.error("导入ElasticSearch中的商品失败，失败原因参数为空：{}", elSearchItemJson);
            return;
        }
        Boolean flag = this.goodsService.importGoodsSoon(elSearchItemJson);
        if (flag) {
            log.info("异步删除ElasticSearch中的商品成功，消息消费成功");
        } else {
            log.error("异步删除ElasticSearch中的商品失败，消息消费失败");
